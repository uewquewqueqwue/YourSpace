...
export async function initializeTodoStore() {
  ...
  if (token && isAuthenticated()) await syncFromServer(token)
}
...
const forceSync = async (token?: string) => {
  ...
  for (const todo of todos.value) {
    if (!todo.id.startsWith('local-')) {
      await window.electronAPI.todos.update(t, todo.id, {
      ...
      if (todo.tags?.length) {
        for (const t of todo.tags) {
          if (t.tag.id.startsWith('local-')) {
            const serverTag = await window.electronAPI.todos.tags.create(token!, {
              name: t.tag.name,
              color: t.tag.color
            })
            tagIds.push(serverTag.id)
          } else {
            tagIds.push(t.tag.id)
          }
        }
      }

      const serverTodo = await window.electronAPI.todos.create(t!, {
        title: todo.title,
        description: todo.description || undefined,
